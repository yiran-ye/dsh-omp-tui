...
function context(status: AgentStatus, input = '') {
  return {
    ...
    cycleReasoningEffort: vi.fn<() => void>(),
    toggleCollaborationMode: vi.fn<() => void>(),
    exit: vi.fn<() => void>(),
...
describe('输入策略与 Slash Commands', () => {
  ...
  it('输入 / 时列出全部已注册命令', async () => {
    ...
    expect(sandboxModes?.items.map((item) => item.value)).toEqual(['danger-full-access'])
  })
  ...
  it('优先显示 Harness 注册表命令，随后是本地命令和可调用 Skills', () => {
    ...
    expect(formatHelpText(commands)).toContain('Ctrl+R 切换思考等级')
    expect(formatHelpText(commands)).toContain('Shift+Tab 切换 Normal/Plan')
  })
  ...
  it('Ctrl+R 切换思考等级并消费按键', () => {
    const policy = new InputPolicy()
    const state = context('idle')

    expect(policy.handle('\u001b[114;5u', state)).toEqual({ consume: true })
    expect(policy.handle('\u001b[114;5:3u', state)).toEqual({ consume: true })
    expect(state.cycleReasoningEffort).toHaveBeenCalledOnce()
  })

  it('Shift+Tab 切换 Normal 与 Plan 并消费按键', () => {
    const policy = new InputPolicy()
    const state = context('idle')

    expect(policy.handle('\u001b[Z', state)).toEqual({ consume: true })
    expect(state.toggleCollaborationMode).toHaveBeenCalledOnce()
  })
